...
def _oversize_json_line_script(payload_size: int = 80 * 1024) -> str:
    ...
    notification = json.dumps({
        ...
        "params": {"sessionId": "ses_test", "update": {"sessionUpdate": "agent_message_chunk"}},
    })
    return (
        f"import sys\nsys.stdout.write({line!r})\nsys.stdout.write('\\n')\n"
        f"sys.stdout.write({notification!r})\nsys.stdout.write('\\n')\nsys.stdout.flush()\n"
    )


@pytest.mark.asyncio
async def test_connection_persists_oversize_frame_and_continues(tmp_path, monkeypatch) -> None:
    ...
    async with spawn_stdio_transport(sys.executable, "-c", script) as (reader, writer, _process):
        ...
        conn = Connection(
            _handler, writer, reader, listening=False, observers=[lambda event: observed.append(event.message)]
        )
    ...
    assert any(msg.get("method") == "session/update" for msg in observed)
